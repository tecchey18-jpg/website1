// ===== MediaGrab Pro - API Service =====
// Connects the frontend to the backend server for real downloads
...
window.fetchMediaReal = async function () {
    ...
    if (health.status !== 'ok') {
        showToast('error', 'Backend server is not running. Using demo mode.');
        return originalFetchMedia();
    }

    if (!health.ytdlp_available) {
        showToast('error', 'yt-dlp not available on server. Using demo mode.');
        return originalFetchMedia();
    }
...
window.downloadMediaReal = async function () {
    ...
    const health = await mediaGrabAPI.healthCheck();
    if (health.status !== 'ok') {
        showToast('warning', 'Server offline. Using demo mode.');
