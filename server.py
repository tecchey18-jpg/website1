...
def get_ytdlp_path():
    ...
    paths = ['yt-dlp', 'yt-dlp.exe', './yt-dlp', './yt-dlp.exe', '/usr/local/bin/yt-dlp', '/usr/bin/yt-dlp']
    for path in paths:
        try:
            # Check version to verify it works
            result = subprocess.run([path, '--version'], capture_output=True, text=True)
            if result.returncode == 0:
                print(f"Found working yt-dlp at: {path}")
                return path
...
def fetch_video_info(url, platform='youtube'):
    ...
    try:
        # Use the Brutal Strategy Engine
        _, info = find_working_strategy(url, ytdlp)
...
def stream_download():
    ...
    # Use strategy for streaming too
    try:
        strategy_args, _ = find_working_strategy(url, ytdlp)
        cmd.extend(strategy_args)
    except:
